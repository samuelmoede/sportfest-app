...
[x] Gruppenbildung für KO-Runden generalisieren statt hart auf 6/7 Teams kodiert. Update 2026-09-13 (Issue #101, Teilschritt 1): das faire Rundenverfahren selbst (`_generate_balanced_pairings`) ist bereits generisch für beliebige Teamzahl (dauerhaft per `tests/test_schedule_generator_robustness.py` für 3-32 Teams/gerade+ungerade abgesichert, vorher nur empirisch für N=4-13 geprüft). Update 2026-09-13 (Issue #101, Teilschritt 2): Turniermodus-Auswahl fuer Wettbewerbe vom Typ "Turnier" ergaenzt - "Gruppenphase mit KO-Runde" (bisheriges Verhalten, Default fuer Bestandsdaten) und neu "Reine KO-Runde" (direktes Einzelausscheiden ohne Gruppenphase), als eigenstaendiges Strategie-Modul unter `app/services/tournament_modes/` (Registry `TURNIER_MODES`, analog zur bestehenden `SCHULPOKAL_MODES`-Registry). `ko_runde.py::build_ko_rounds()` baut dabei einen vollstaendigen, lueckenlosen KO-Baum fuer beliebige Teamzahl (>= 2) inkl. fairer Freilos-Vergabe per Standard-Turniersetzung, dauerhaft abgesichert fuer 2-32 Teams in `tests/test_ko_runde_service.py`. Nutzt die bereits bestehende, bisher nur fuer "Schulpokal" befuellte Spalte `competitions.tournament_mode` mit (statt einer zusaetzlichen `turnier_modus`-Spalte) - bleibt additiv/rueckwaertskompatibel, da NULL weiterhin als Default interpretiert wird. Bewusst nicht enthalten: automatische Weiterbesetzung nachfolgender KO-Runden aus den Ergebnissen der vorigen bei mehr als 4 Teams (die bestehenden "Halbfinale"/"Finale automatisch besetzen"-Aktionen bleiben auf den Modus "Gruppenphase mit KO-Runde" beschraenkt) - Folgerunden werden vorerst manuell eingetragen, vorgesehen als Teilschritt 3
[x] Echte Team-Doppelbelegung bei vielen Teams/wenigen Feldern gefunden durch den neuen Robustheits-Test (2026-09-13, Issue #101): `generate_group_plan`/`generate_schulpokal_plan` erhöhten die Rundenzeit über ein echtes `datetime`-Objekt (`current_time += timedelta(...)`, Anzeige per `strftime("%H:%M")`). Reicht die Rundenzahl bei wenigen Feldern und vielen Teams (z.B. "jeder gegen jeden" mit 25+ Teams auf nur 2 Feldern - schon rein rechnerisch >24h Spielzeit) über einen vollen 24h-Tag hinaus, springt `strftime` klanglos auf `00:00` zurück, wodurch eine spätere Runde exakt denselben Uhrzeit-String wie eine frühere bekam - da `slots.startzeit` kein Datum kennt, wurde das dann korrekt als Team-Doppelbelegung erkannt (bestätigt: `test_full_round_robin_gives_every_team_teamcount_minus_one_games` schlug für 25-32 Teams bei 2 Feldern fehl, für ≤24 Teams nie, exakt der Punkt an dem die Gesamtrundenzahl × Rundenintervall erstmals 24h übersteigt). Fix: neue Hilfsfunktion `_minutes_to_clock()` verfolgt die Rundenzeit als reine Minutenzahl seit Mitternacht statt über ein wrap-anfälliges `datetime`-Objekt - Stunden werden dabei nie auf 24 zurückgesetzt, jede Runde bekommt garantiert einen eindeutigen Zeit-String, unabhängig von der Rundenzahl. Für alle realistischen Konfigurationen (Spielplan bleibt unter 24h) identisches Verhalten wie zuvor
